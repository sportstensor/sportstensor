...
class MLSSoccerPredictionModel(SoccerPredictionModel):
    ...
    def get_data(self) -> pd.DataFrame:
        ...
        file_path = hf_hub_download(
            repo_id=self.huggingface_model, filename=self.mls_fixture_data_filepath
        )
    ...
    def prep_pred_input(
        ...
        file_path = hf_hub_download(
            repo_id=self.huggingface_model, filename=self.mls_combined_table_filepath
        )
        fixture_data_file_path = hf_hub_download(
            repo_id=self.huggingface_model, filename=self.mls_fixture_data_filepath
        )
        ...
        if date_formatted.date() < current_date:
            ...
            if not os.path.exists(fixture_data_file_path):
                ...
                input = matching_input[["HT", "AT", "HT_GD", "AT_GD"]].values.astype(
                    float
                )  # , 'HT_ELO', 'AT_ELO'
