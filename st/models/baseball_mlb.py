...
class MLBBaseballPredictionModel(BaseballPredictionModel):
    ...
    def prep_pred_input(
        ...
        file_path = hf_hub_download(
            repo_id=self.huggingface_model, filename=self.mlb_current_team_data_filepath
        )

        current_team_stats = pd.read_excel(file_path)

        home_abrv = current_team_stats[current_team_stats["team"] == home_team][
            "abrv"
        ].values[0]
        away_abrv = current_team_stats[current_team_stats["team"] == away_team][
            "abrv"
        ].values[0]
        ...
        if date_formatted.date() < current_date:
            ...
            if not os.path.exists(file_path):
                ...
                try:
                    ...
                    matching_input = 0
                    print(
                        "Match could not be found in data source, scrape more data or check inputs."
                    )
            ...
            input["HT_WL_RATIO"] = (
                home_stats["w"].to_numpy()[0] / home_stats["l"].to_numpy()[0]
            )
            input["AT_WL_RATIO"] = (
                away_stats["w"].to_numpy()[0] / away_stats["l"].to_numpy()[0]
            )
