...
class MinerEvaluator:
    ...
    def __init__(self, config: bt.config, uid: int, metagraph_syncer: MetagraphSyncer):
        ...
        self.scorer = MinerScorer(self.metagraph.n)
    ...
    async def eval_miner(self, uid: int) -> None:
        ...
        Specifically:
            1. Finds scorable Predictions from miners that have been waiting for the match results
            2. Passes the validation result to the scorer to update the miner's score.
        """
