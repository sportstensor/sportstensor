...
def calculate_incentives_and_update_scores():
    ...
    # Dictionaries to store component scores for normalization
    final_signal_qualities: Dict[League, List[float]] = {league: [0.0] * len(all_uids) for league in ACTIVE_LEAGUES}
    final_edge_scores: Dict[League, List[float]] = {league: [0.0] * len(all_uids) for league in ACTIVE_LEAGUES}
    final_roi_scores: Dict[League, List[float]] = {league: [0.0] * len(all_uids) for league in ACTIVE_LEAGUES}
    ...
    for league in leagues_to_analyze:
        ...
        for index, uid in enumerate(all_uids):
            ...
            # Calculate ROI returns for Sharpe calculation
            roi_returns = []
            for pwmd in predictions_with_match_data:
                if pwmd.prediction.get_predicted_team() == pwmd.get_actual_winner():
                    roi_return = pwmd.get_actual_winner_odds() - 1
                    roi_returns.append(roi_return)
                else:
                    roi_return = -1  # Lost the bet amount
                    roi_returns.append(roi_return)
                
                # Store the ROI return in the pwmd object
                pwmd.roi_return = roi_return

            # Sharpe (over ROI-based returns)
            roi_sharpe = calculate_sharpe_ratio(roi_returns)
            
            # Step 1: Signal quality score from ROI Sharpe
            signal_quality = max(0.0, roi_sharpe)

            # Store values for later normalization
            final_signal_qualities[league][index] = signal_quality
            final_edge_scores[league][index] = total_score  # Store without rho
            final_roi_scores[league][index] = final_roi_score

            # Apply raw weighted score for display purposes
            raw_score = league_scores[league][index]  # This already includes rho

            # Save penalized score
            league_scores[league][index] = raw_score
        ...
        if league_table_data:
            ...
            print(tabulate(league_table_data, headers=['UID', 'Edge Score', 'ROI Score', 'ROI', 'Mkt ROI', 'ROI Diff', 'ROI Incr', 'Mkt ROI Incr', 'Max ROI Incr', '# Preds', 'Rho', '# Lay Preds', '# Underdog Preds', 'Raw Edge', 'ROI Sharpe'], tablefmt='grid'))
        else:
        ...
        min_edge, max_edge = min(final_edge_scores[league]), max(final_edge_scores[league])
        ...
        if max_edge - MIN_EDGE_SCORE == 0:
            ...
            normalized_edge = []
            for i, score in enumerate(final_edge_scores[league]):
        ...
        min_roi, max_roi = min(final_roi_scores[league]), max(final_roi_scores[league])
        if max_roi - min_roi == 0:
            normalized_roi = [0 for score in final_roi_scores[league]]
        else:
            normalized_roi = [(score - min_roi) / (max_roi - min_roi) if (max_roi - min_roi) > 0 else 0 for score in final_roi_scores[league]]

        # Apply the final normalized calculation to league_scores
        for i in range(len(all_uids)):
            league_scores[league][i] = (
                0.4 * normalized_edge[i] +
                0.4 * normalized_roi[i] +
                0.2 * (final_signal_qualities[league][i] / max(1e-5, max(final_signal_qualities[league])))
            ) * league_rhos[league][i]
        
        # The previous raw score calculations have now been replaced with properly normalized scores
